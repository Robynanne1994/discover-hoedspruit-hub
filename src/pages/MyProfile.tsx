  ...
  const { data: saved } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: listings } = await supabase
        .from("listings")
        .select(`id, title, title_override, location, google_rating, google_reviews_count, category_id, opening_hours, opening_hours_label, additional_hours, categories(title), ${LISTING_IMAGE_COLUMNS}`)
        .in("id", ids);
