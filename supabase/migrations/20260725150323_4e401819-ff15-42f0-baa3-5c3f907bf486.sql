...
-- Let users read their own moderation actions
...
-- Update moderation function to link notifications to the new page
CREATE OR REPLACE FUNCTION public.apply_moderation_action(_report_id uuid, _action text, _severity text DEFAULT NULL::text, _duration_days integer DEFAULT NULL::integer, _admin_note text DEFAULT NULL::text, _notify_reporter_message text DEFAULT NULL::text, _target_user_id uuid DEFAULT NULL::uuid)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF v_target IS NOT NULL AND _action <> 'dismissed' THEN
    ...
    VALUES
      (v_target, 'moderation', 'unread', v_title, v_body, '/account-notices', 'user_reports', _report_id);
  END IF;
