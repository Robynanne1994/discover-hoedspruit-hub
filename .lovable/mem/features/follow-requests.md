...
- Notification avatars: `useFollowRequestActors` takes `{ ref_id, link }` per notification. Decline DELETEs the follows row, so `ref_id` (a `follows.id`) no longer resolves — the hook falls back to the profile id parsed out of the notification's `/profile/<uuid>` link, and merges results into state instead of replacing them, so an avatar already on screen never blanks out. The accept/decline optimistic update must mirror `link` too, or the declined card loses its picture until the realtime refresh lands. Any new follow-related notification kind belongs in `FOLLOW_ACTOR_KINDS`.
